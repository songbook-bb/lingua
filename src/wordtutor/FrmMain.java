...
public class FrmMain extends JFrame implements ActionListener,KeyListener
{
	...
	public void createTable()
	{
		...
		tableWords.addMouseListener(new MouseAdapter() { 
          public void mousePressed(MouseEvent me) {
            ...
            if (column == 5 && (Integer)tableWords.getValueAt(row, column) > 0) {
              String complexFileName =  tutor.getCurrentDictFile().substring(0, tutor.getCurrentDictFile().indexOf("."))+
              File.separator+(Integer)tableWords.getValueAt(row, column)+Util.getAppProperty("MP3.EXTENSION");                          
	...
	private void startLesson()
	{
	    ...
	    if(tutor.isWordListNotEmpty() == Util.T) {			
		 ...
		 try {
			 new FrmLesson(this, tutor.getSettings().isExamMode() ? MessageFormat.format(Util.getLocalizedString("MAIN.EXAM.TITLE"), tutor.getCurrentDictFile()) : MessageFormat.format(Util.getLocalizedString("MAIN.LESSON.TITLE"), tutor.getCurrentDictFile()), tutor);
			 tutor.fillTableModel();
			 tableWords.setVisible(true);
			 tableWords.repaint();
			 tutor.saveToXML();			 			 
		 } catch (Exception e) {
			 JOptionPane.showMessageDialog(this, e.getMessage(), Util.getLocalizedString("MAIN.LESSON.BAD.CONFIG.TITLE"), JOptionPane.WARNING_MESSAGE);
		 }
		} else if (tutor.isWordListNotEmpty() == Util.F) {
			JOptionPane.showMessageDialog(this, Util.getLocalizedString("MAIN.LESSON.WARNING.MESSAGE"),Util.getLocalizedString("MAIN.LESSON.WARNING.TITLE"),JOptionPane.WARNING_MESSAGE);
		} else if (tutor.isWordListNotEmpty() == Util.E) {
			JOptionPane.showMessageDialog(this, Util.getLocalizedString("MAIN.LESSON.BAD.CONFIG.MESSAGE"), Util.getLocalizedString("MAIN.LESSON.BAD.CONFIG.TITLE"), JOptionPane.WARNING_MESSAGE);
		}				
	}
	...
	{			
		TableRowSorter<WordsTableModel> sorter = new TableRowSorter<WordsTableModel>(tableModel);
	    ...
	    RowFilter<Object,Object> bothRowsContainExpressionFilter = new RowFilter<Object,Object>() {
	      public boolean include(Entry<? extends Object, ? extends Object> entry) {
	        for (int i = entry.getValueCount() - 1; i >= 0; i--) {
	          if (entry.getStringValue(i).toUpperCase().indexOf(searchString.toUpperCase()) > -1) { 
	            return true;
	          }
	        }
	        return false;
	...
	private void infoAbout() {	
		JOptionPane.showMessageDialog(this, Util.getLocalizedString("PROGRAM.VERSION") +" "+ Util.programVersion + Util.NEW_LINE_DELIMITER +Util.getLocalizedString("PROGRAM.AUTHOR")+ " "+ Util.authorInfo + Util.NEW_LINE_DELIMITER +Util.getLocalizedString("PROGRAM.CONTACT")+ " " + Util.contactEmail
				+ Util.NEW_LINE_DELIMITER + Util.getLocalizedString("PROGRAM.HELPED.A.LOT")+ " " + Util.thanksList + Util.NEW_LINE_DELIMITER+Util.NEW_LINE_DELIMITER+Util.getLocalizedString("PROGRAM.MY.THANKS"), 
				Util.getLocalizedString("MAIN.ABOUT.TITLE"), JOptionPane.INFORMATION_MESSAGE);
	}		
	...
	{			
		...
		if (!Util.isEmpty(switchToLessonFile)) {			 	    
			...
			tutor.assignTableModel(tableModel);
			tutor.fillTableModel();
			setTitle(MessageFormat.format(Util.getLocalizedString("MAIN.TITLE"), switchToLessonFile));			
		}		
		tableWords.setVisible(true);
		tableWords.repaint();
	}		
	...
	{
		new FrmImport(this, Util.getLocalizedString("FRAME.IMPORT.LABEL"), tutor, tableWords);
	}	

	public void importXLS()
	{
		new FrmImportXls(this, Util.getLocalizedString("FRAME.XLS.LABEL"), tutor, tableWords);
	}	
	...
	public void newDictionary() 
	{
		...
		s = s+".naq";
		/**
		 * @TODO Czy takiego pliku już nie ma? 
		 * I sprawdzenie czy nazwa s jest poprawnym plikiem
		 */
		try {
			 File fl = new File(s);			
			 if (fl.exists()){
				 throw new Exception(MessageFormat.format(Util.getLocalizedString("MAIN.ERROR.FILE.EXISTS"),s)); 
			 }			 
			createNewDictFile(s);
		} catch (Exception eio) {
			JOptionPane.showMessageDialog(this, eio.getMessage(),Util.getLocalizedString("ERROR.DIALOG.TITLE"),JOptionPane.ERROR_MESSAGE);
			eio.printStackTrace();			
			return;
		}  		
		switchLesson(s);
 		 ...
 		 tutor.buildCurrentSoundSampleCoverMapIndex();		   	     
			...
			// show message
   			JOptionPane.showMessageDialog(
   	   	            this,
   	   	            Util.getLocalizedString("MAIN.EDIT.NONE.SELECTED.WINDOW.MESSAGE")+Util.NEW_LINE_DELIMITER,		                    
	...
	public void keyReleased(KeyEvent e)
	{
      switch (e.getKeyCode())
      {
       case KeyEvent.VK_ENTER  :  startLesson();
       case KeyEvent.VK_F1 : 
	...
	public String chooseLessonNaqFile() {	 
		 ...
		 Collection<File> fList =  (Collection<File>)FileUtils.listFiles(new File(WordTutor.LESSONS_DIR), extensions, false);
		 ArrayList<String> stringList = new ArrayList<String>();
		 if (fList.size() == 0) {
			JOptionPane.showMessageDialog(this, Util.getLocalizedString("MAIN.ERROR.MESSAGE"),Util.getLocalizedString("ERROR.DIALOG.TITLE"),JOptionPane.ERROR_MESSAGE);
			this.dispose();
			System.exit(0);
		 }
		 for (File f : fList ) {
		 ...
		 String chooseLessonNaqFile = (String)JOptionPane.showInputDialog(
		                     this,
	...
	{	 	
	 super(title);
	 String chooseLessonNaqFile = chooseLessonNaqFile();	 
     ...
     tutor.loadFromXML();
     // UWAGA : moze nie najlepsze miejsce - ale na razie tu buduje indeks sampli
     tutor.buildCurrentSoundSampleCoverMapIndex(); 
   	 setTitle(MessageFormat.format(Util.getLocalizedString("MAIN.TITLE"), chooseLessonNaqFile));
	 ...
	 startLesson = new JButton(tutor.getSettings().isExamMode() ? Util.getLocalizedString("MAIN.EXAM") : Util.getLocalizedString("MAIN.LESSON"));
	 startLesson.setActionCommand("startLesson");
	 startLesson.setToolTipText(tutor.getSettings().isExamMode() ? Util.getLocalizedString("MAIN.EXAM.TIP") : Util.getLocalizedString("MAIN.LESSON.TIP"));
	 ...
	 switchLesson = new JButton(Util.getLocalizedString("MAIN.SWITCH.LESSON"));
	 switchLesson.setActionCommand("switchLesson");
	 switchLesson.setToolTipText(Util.getLocalizedString("MAIN.SWITCH.LESSON.TIP"));
	 panelManage.add(switchLesson);
	 switchLesson.addActionListener(this);	 	 
	 	 
	 buttonXlsImport = new JButton(Util.getLocalizedString("MAIN.XLS.IMPORT"));
	 buttonXlsImport.setActionCommand("xlsImport");
	 buttonXlsImport.addActionListener(this);
	 buttonXlsImport.setToolTipText(Util.getLocalizedString("MAIN.XLS.IMPORT.TIP"));	 
	 panelManage.add(buttonXlsImport);	 
	 
	 buttonXlsExport = new JButton(Util.getLocalizedString("MAIN.XLS.EXPORT"));
	 buttonXlsExport.setActionCommand("xlsExport");
	 buttonXlsExport.addActionListener(this);
	 buttonXlsExport.setToolTipText(Util.getLocalizedString("MAIN.XLS.EXPORT.TIP"));	 
	 panelManage.add(buttonXlsExport);	 
	 
//TODO usun jak bedzie juz kompletnie niepotrzebny import z pliku TXT
	 
//	 buttonImport = new JButton(Util.getLocalizedString("MAIN.IMPORT"));
//	 buttonImport.setActionCommand("import");
//	 buttonImport.addActionListener(this);
//	 buttonImport.setToolTipText(Util.getLocalizedString("MAIN.IMPORT.TIP"));	 
//	 panelManage.add(buttonImport);	 
	...
	{			
		...
		logger.debug("Mp3 conversion is "+((wave2mp3Result == 0)?"OK":"NOT OK"));
		if (Util.TRUE.equalsIgnoreCase(Util.getAppProperty("PLAY.INTRO"))) {
	        Util.playSound(player, Util.getAppProperty("INTRO.MP3"));		  
		}				
		try {
		    JFrame mainWindow = new FrmMain(Util.getLocalizedString("MAIN.FRAME.TITLE"));		    
...
//		    row.createCell(1).setCellValue(Util.getLocalizedString("WTM.NATIVE"));		    
		    ...
		    for (int i=0; i < tutor.getMaxWordCount(); i++) {
			    ...
			    row.createCell(0).setCellValue(word.getForeignWord());
			    String[] splitted = Util.improvedSplitter(word.getNativeWord(),Util.PHRASE_DELIMITER);
			    if (splitted.length > maxSplit) {
			    	maxSplit = splitted.length;
			    }
			    for (int j = 0; j < splitted.length; j++) {
				    row.createCell(j+1).setCellValue(splitted[j]);			    	
			    }
		    }
		    for (int x = 0; x <= maxSplit; x++) {
		    	sheet.autoSizeColumn(x);	//auto adjust width of the x-th column
		    }
		    FileOutputStream fileOut = new FileOutputStream(tutor.getCurrentDictFile()+Util.XLS_EXTENTION);
		    wb.write(fileOut);
		    fileOut.close();
   			JOptionPane.showMessageDialog(
   	   	            this,
   	   	            MessageFormat.format(Util.getLocalizedString("MAIN.XLS.EXPORT.FRAME.MESSAGE"), tutor.getCurrentDictFile(), System.getProperty("user.dir")+File.separator+tutor.getCurrentDictFile()+Util.XLS_EXTENTION),  
   	   	            Util.getLocalizedString("MAIN.XLS.EXPORT.FRAME.NAME"),
   	   	            JOptionPane.PLAIN_MESSAGE,
   	   	            null);
		    
	    } catch (Exception e) {
			JOptionPane.showMessageDialog(this, e.getMessage(), Util.getLocalizedString("ERROR.DIALOG.TITLE"),JOptionPane.ERROR_MESSAGE);
