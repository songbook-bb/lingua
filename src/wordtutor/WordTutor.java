...
public class WordTutor implements IWTSerializable {
  ...
  /**
   * default constructor loads the settings from XML and applies it for the word
   * storage
   */
...
//    buildCurrentSoundSampleCoverMapIndex();    
  ...
  public void loadFromXML() {
    ...
    int cnt = dWords.size();
    for (int i = 0; i < cnt; i++) {
      IWord newWord = words.createWord(dWords.get(i).getForeignWord(), dWords.get(i).getNativeWord(), dWords.get(i)
          .getIdSound());
  ...
  public void saveToXML() {
    ...
    dType.setKeyboard(settings.keyboardEnumerationToInt(settings.getKeyboardType()));
  ...
  /**
   * opens the window for changing the settings apply word setting for a storage
   */
  public void openSettings(FrmMain parentFrame) {
    new FrmSettings(parentFrame, Util.getLocalizedString("WT.SETTINGS.TITLE"), settings);
  }
...
//	  	logger.debug("sndID:"+this.getIdSound());	  	
	  	if (this.getIdSound() > 0 && getCurrentSoundSampleCoverMap().get(this.getIdSound()) != null && getCurrentSoundSampleCoverMap().get(this.getIdSound()) >= 0) {
		  	int chooseNameInt =  (getCurrentSoundSampleCoverMap().get(this.getIdSound()) > 0) ? (Math.abs(r.nextInt()) % getCurrentSoundSampleCoverMap().get(this.getIdSound())) : 0;
		  	String nameLetter = "";
		  	if (chooseNameInt > 0) {
		  		nameLetter = Character.toString((char)(96+chooseNameInt));
		  	}
			String complexFileName = this.getCurrentDictFile().substring(0, this.getCurrentDictFile().indexOf(".")) + File.separator + this.getIdSound() + nameLetter + Util.getAppProperty("MP3.EXTENSION");
	 ...
	 * 								 value (is number of sound mp3 samples associated with the phrase)  
	 * 
	 * @return
	 */
	public void buildCurrentSoundSampleCoverMapIndex() {
		...
		if (lessonSampleDirectory.isDirectory()) {
			@SuppressWarnings("unchecked")		
			LinkedList<File> fList = (LinkedList<File>)FileUtils.listFiles(lessonSampleDirectory, extensions, false);
		...
		for (File mp3File : strangeCastNotWorkedList) {
			namesWithoutExtensionList.add(mp3File.getName().substring(0, mp3File.getName().indexOf(Util.FULL_STOP)));
		}
		for (int i = 1; i <= maxWordsInLesson; i++) {
			for (int j = 1; j < maxSamplesCounter; j++) {
				if (j == 1) {
					...
					if (namesWithoutExtensionList.contains("" + i+""+Character.toString((char)letterValue))) {
						sampleMap.put(i, j);
					} else {
						break;
					}
  ...
  public void setAllWordsCapitalLetter() {
	  for (int i=0; i < words.size(); i++) {
		  ...
		  nativeWord = nativeWord.substring(0,1).toUpperCase()+nativeWord.substring(1, nativeWord.length());
		  String foreignWord = words.get(i).getForeignWord();
		  foreignWord = foreignWord.substring(0,1).toUpperCase()+foreignWord.substring(1, foreignWord.length());		  
  ...
  public void addNewWord() {
    ...
    try {
      String fileWave = getMaxIdSound() + Util.getAppProperty("WAVE.EXTENSION");
      String waveFullPath = Util.getAppProperty("RESOURCE.PATH") + File.separator
          + Util.getAppProperty("MP3.PATH") + File.separator
          + getCurrentDictFile().substring(0, getCurrentDictFile().indexOf("."));
      ;
      ...
      try {
          fAddWord = new FrmAddWord(null, Util.getLocalizedString("WT.ADDNEW.TITLE"), wordData, waveFullPath
                  + File.separator + fileWave);
  ...
  public int getMaxIdSound() {
    int tmpId = 0;
    for (int i = 0; i < words.size(); i++) {
      tmpId = (words.get(i).getIdSound() > tmpId) ? words.get(i).getIdSound() : tmpId;
    }
    tmpId++;
    return tmpId;
  ...
  public void editWord(int index) {
    ...
    while (words.size() > index && lastAddedSound) {
      ...
      /**
       * @TODO Edycja do sprawdzenie czy nie usuwa podłączenia plików Sound
       * 
       */
      String fileWave;
      ...
      try {
          try {
              FrmAddWord fEditWord = new FrmAddWord(null, Util.getLocalizedString("WT.EDIT.TITLE"), wordData, waveFullPath
                  + File.separator + fileWave);
      ...
      if (wordData.isSaved) {
        ...
        /**
         * @TODO Może trzeba pokazać okno - gdy ustawiono jakiś w warningów
         * 
         */
  ...
  public void mergeWords(int indexes[]) {
    ...
    if (cnt == 2) {
    	String fullForeign = words.get(indexes[0]).getForeignWord()+Util.PHRASE_DELIMITER+words.get(indexes[1]).getForeignWord();
    	String fullNative = words.get(indexes[0]).getNativeWord()+Util.PHRASE_DELIMITER+words.get(indexes[1]).getNativeWord();
      	words.get(indexes[0]).setForeignWord(removeDuplicatedPhrasesWithinString(fullForeign));
      	words.get(indexes[1]).setForeignWord(removeDuplicatedPhrasesWithinString(fullForeign));
      	words.get(indexes[0]).setNativeWord(removeDuplicatedPhrasesWithinString(fullNative));
      	words.get(indexes[1]).setNativeWord(removeDuplicatedPhrasesWithinString(fullNative));    	
    }
  }
  ...
  public String removeDuplicatedPhrasesWithinString(String multiString) {
		String[] ffArray = Util.improvedSplitter(multiString,Util.PHRASE_DELIMITER);
...
//	  	}
  ...
  public void delWord(int indexes[]) {
    ...
    // find & remove duplicates (whole same example = question & answer are duplicated)
    int i = 0;
    int j = 0;
    String nativeString = "";
  ...
  public String getRightAnswer() {
    String result = (direction == TranslateDirection.STRAIGHT) ? getIndexedWord(wordIndex).getNativeWord()
  ...
  /**
   * starts a new lesson. clears and fills the indexer of the words used in the
   * lesson
   */
  public void startLesson() throws Exception {
    ...
    if (settings.isHardest() && settings.isRandom()) {
    	throw new Exception("Cannot configure hardest and random one at a time.");
    }
  ...
  /**
   * assigns the table model. also it assigns word storage of the tutor to model
   */
  ...
  /** Util.T - if there are some words 
   * check if the dictionary contains any words which are not learned
   */
  /**
   * 
   */
  int isWordListNotEmpty() {
    ...
    if (result == Util.T) {
	  ...
	  int helpCount = isRH ? (settings.getWordsInLesson()) : settings.getWordsInLesson()+ settings.getIncScore() - 1;
  ...
  public String calcLessonStatistics() {
	  ...
	  return "["+countGood+"/"+words.size()+"] = "+(100*countGood/words.size());
  }
  ...
  public String calcExamStatistics() {
	  ...
	  return "["+countGood+"/"+(countGood+countBad)+"] = "+(100*countGood/(countGood+countBad));
  }
...
public void setCurrentSoundSampleCoverMap(HashMap<Integer, Integer> currentSoundSampleCoverMap) {
