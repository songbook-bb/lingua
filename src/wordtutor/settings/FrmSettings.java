...
public class FrmSettings extends JDialog implements ActionListener {
	...
	private void Save() throws WrongNumberException
	{
		...
		catch (Exception e)
		{
			...
			throw new WrongNumberException(Util.getLocalizedString("SETTINGS.EXCEPTION.INVALID"));
		}
		if (wordsInLesson<1) {
			throw new WrongNumberException(Util.getLocalizedString("SETTINGS.EXCEPTION.NUMBER"));
		}			
		if (incScore<1) {
			throw new WrongNumberException(Util.getLocalizedString("SETTINGS.EXCEPTION.FIRST"));
		}			
    ...
    if (cbRandom.isSelected()) {
		...
		parentFrame.getStartLesson().setText(settings.isExamMode() ? Util.getLocalizedString("MAIN.EXAM") : Util.getLocalizedString("MAIN.LESSON"));
		parentFrame.getStartLesson().setToolTipText(settings.isExamMode() ? Util.getLocalizedString("MAIN.EXAM.TIP") : Util.getLocalizedString("MAIN.LESSON.TIP"));
    ...
    {        	
     ...
     cbStrictMode = new JCheckBox(Util.getLocalizedString("SETTINGS.CHECKBOX.STRICT"));
     cbStrictMode.setSelected(settings.isStrictMode());
     cbStrictMode.setToolTipText(Util.getLocalizedString("SETTINGS.CHECKBOX.STRICT.TIP"));
     panelSettings.add(cbStrictMode);
          
     cbHardest = new JCheckBox(Util.getLocalizedString("SETTINGS.CHECKBOX.HARDEST"));
     cbHardest.setSelected(settings.isHardest());     
     cbHardest.setToolTipText(Util.getLocalizedString("SETTINGS.CHECKBOX.HARDEST.TIP"));     
     panelSettings.add(cbHardest);

     CheckBoxListener	checkBoxListener = new CheckBoxListener();
     cbHardest.addItemListener(checkBoxListener);

     cbRandom = new JCheckBox(Util.getLocalizedString("SETTINGS.CHECKBOX.RANDOM"));
     cbRandom.setSelected(settings.isRandom());     
     cbRandom.setToolTipText(Util.getLocalizedString("SETTINGS.CHECKBOX.RANDOM.TIP"));     
     panelSettings.add(cbRandom);     
     cbRandom.addItemListener(checkBoxListener);     

     cbUnlearned = new JCheckBox(Util.getLocalizedString("SETTINGS.CHECKBOX.TAKE.UNFLAGGED"));
     cbUnlearned.setSelected(settings.isUnlearned());
     cbUnlearned.setToolTipText(Util.getLocalizedString("SETTINGS.CHECKBOX.TAKE.UNFLAGGED.TIP"));
     panelSettings.add(cbUnlearned);       
     //@TODO REMOVE THIS JLABEL just to make items EVEN
     // WARNING (DUMP JLABEL)
     ...
     labDirection = new JLabel(Util.getLocalizedString("SETTINGS.LABEL.DIRECTION"));
     panelDirection.add(labDirection);     
     rbStraight = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.STRAIGHT"));
     panelDirection.add(rbStraight);
     rbReverse = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.REVERSE"));
     panelDirection.add(rbReverse);          
     rbBoth = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.BOTH"));
     ...
     {
       case STRAIGHT: rbStraight.setSelected(true);break; 
       case REVERSE: rbReverse.setSelected(true);break;
       case BOTH: rbBoth.setSelected(true);break;
     }    	
     
     // Additional keyboard Group     
     panelKeyboards = new JPanel();
     panelKeyboards.setLayout(new BoxLayout(panelKeyboards, BoxLayout.Y_AXIS));     
     labKeyboardButton = new JLabel(Util.getLocalizedString("SETTINGS.LABEL.KEYBOARDS"));
     panelKeyboards.add(labKeyboardButton);     
     rbSpanish = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.SPANISH"));
     panelKeyboards.add(rbSpanish);
     rbGerman = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.GERMAN"));
     panelKeyboards.add(rbGerman);          
     rbKashubian = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.KASHUBIAN"));
     panelKeyboards.add(rbKashubian);
     rbPolish = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.POLISH"));
     panelKeyboards.add(rbPolish);
     rbItalian = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.ITALIAN"));
     panelKeyboards.add(rbItalian);
     rbFrench = new JRadioButton(Util.getLocalizedString("SETTINGS.RADIO.FRENCH"));
     ...
     {
       case SPANISH: rbSpanish.setSelected(true);break; 
       case GERMAN: rbGerman.setSelected(true);break;
       case KASHUBIAN: rbKashubian.setSelected(true);break;
  ...
  class CheckBoxListener implements ItemListener {
      public void itemStateChanged(ItemEvent e) {
    	//logger.debug("Item "+e.getID()+ " "+e.getItem()+" "+e.getSource()); 
    	JCheckBox c = (JCheckBox)(e.getSource());
    	//logger.debug("Item "+c.getText());
    	if (c.isSelected() && Util.getLocalizedString("SETTINGS.CHECKBOX.RANDOM").equals(c.getText())) {
    		cbHardest.setSelected(false);
    	} else if (c.isSelected() && Util.getLocalizedString("SETTINGS.CHECKBOX.HARDEST").equals(c.getText())) {
